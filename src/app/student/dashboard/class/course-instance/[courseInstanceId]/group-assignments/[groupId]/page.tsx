    ...
    <div className="fixed z-50 inset-0 flex items-center justify-center bg-black/70">
      <div className="relative bg-white rounded-2xl shadow-xl p-4 max-w-3xl max-h-[80vh]">
        ...
        <img
          src={src}
          alt={alt || "Preview"}
          className="rounded-xl max-w-[70vw] max-h-[70vh] object-contain"
        />
      </div>
    </div>
  );
...
export default function GroupAssignmentDetail() {
  ...
  const [groupUndoState, setGroupUndoState] = useState<
    Record<string, { loading: boolean; error: string | null }>
  >({});
  const setLoadingUndo = (groupId: string, loading: boolean) =>
    setGroupUndoState((prev) => ({
      ...prev,
      [groupId]: { ...(prev[groupId] || {}), loading },
    }));
  const setUndoError = (groupId: string, error: string | null) =>
    setGroupUndoState((prev) => ({
      ...prev,
      [groupId]: { ...(prev[groupId] || {}), error },
    }));
  const [plagResults, setPlagResults] = useState<Record<string, any>>({});
  const [activeGroupForModal, setActiveGroupForModal] = useState<string | null>(
    null
  );
  ...
  const fetchAssignment = () => {
    ...
    setLoading(true);
    fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/group-assignment/${groupAssignmentId}`,
      {
        headers: {
          Authorization:
            "Bearer " +
      ...
      .then(async (res) => {
        ...
        (data.assignment.groups || []).forEach((g: GroupObj) => {
          expand[g._id] = !!(
            myUserId &&
            g.members.some((m: UserObj) => (m._id || m.id) === myUserId)
          );
        });
  ...
  if (!assignment) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-gray-600 bg-white px-6 py-4 rounded-xl shadow">
          No assignment found.
        </div>
      </div>
    );
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h1 className="text-3xl font-bold">
                    {assignment.title || "Untitled Group Assignment"}
                  </h1>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {assignment.postedBy?.username ||
                          assignment.postedBy?.email ||
                          "Unknown Teacher"}
                      </span>
                    </div>
                    <span>•</span>
                    <span>
                      {assignment.courseInstance?.course} (
                      {assignment.courseInstance?.batch})
                    </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              ...
              {assignment.documents?.length ? (
                <ResourceCard
                  ...
                  title="Documents"
                  items={assignment.documents.map((f) => ({
                    url: getFileUrl(f.url),
                    label: f.originalname,
                  }))}
                  type="doc"
              ...
              {assignment.youtubeLinks?.length ? (
                <ResourceCard
                  ...
                  title="YouTube Links"
                  items={assignment.youtubeLinks.map((l, i) => ({
                    url: l,
                    label: `Video ${i + 1}`,
                  }))}
                  type="youtube"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        ...
                        {group.youtubeLinks?.length ? (
                          <ResourceCard
                            ...
                            items={group.youtubeLinks.map((l, idx) => ({
                              url: l,
                              label: `Video ${idx + 1}`,
                            }))}
                            type="youtube"
                    ...
                    {isUserInGroup && (
                      <div className="mb-8">
                        <GroupSubmissionPanel
                          submission={groupSubmissions[group._id] || null}
                          groupAssignmentId={assignment._id}
                          groupId={group._id}
                          loadingUndo={!!groupUndoState[group._id]?.loading}
                          setLoadingUndo={(loading) =>
                            setLoadingUndo(group._id, loading)
                          }
                          ...
                          getFileIcon={() => <FileText className="w-5 h-5" />}
                          isImage={isImage}
                          isOfficeDoc={isOfficeDoc}
                          isPDF={(name) => /\.pdf$/i.test(name)}
                          setMediaPreview={setImgModal}
                          onPlagiarismCheck={(result) =>
                            onGroupPlagCheck(group._id, result)
                          }
                          canSubmit={canSubmit}
                        />
                      </div>
                        ...
                        {isUserInGroup && (
                          <div className="flex gap-2">
                            <input
                              ...
                              onKeyPress={(e) =>
                                e.key === "Enter" && handlePost(i, group)
                              }
                            />
                            <button
                              ...
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-gray-600">
                                    Messages: {p.messageCount || 0}
                                  </span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <Clock className="w-3 h-3 text-gray-500" />
                                  <span className="text-gray-600">
                                    Time: {p.discussionMinutes || 0}m
                                  </span>
                                </div>
                              </div>
                            </div>
          ...
          if (type === "media" && isImage(it.label)) {
            return (
              <div key={i} className="inline-block mr-3 mb-2">
                ...
                />
                <div className="truncate text-xs mt-1 text-center">
                  {it.label}
                </div>
              </div>
            );
