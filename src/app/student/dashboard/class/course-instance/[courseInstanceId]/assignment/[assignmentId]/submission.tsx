...
import {
  ...
  Eye, Trash2, AlertCircle, Trophy, Star, MessageSquare, ExternalLink, Plus, Lock
} from "lucide-react";
...
export default function SubmissionPanel({
  ...
  acceptingSubmissions = true, closeAt = null
}: Props) {
  ...
  const isPastClose = closeAt ? new Date() > new Date(closeAt) : false;
  const isClosed = !acceptingSubmissions || isPastClose;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (user?._id || (user as any)?.id) {
      formData.append("student", (user as any)._id || (user as any).id);
    }
    formData.append("assignment", assignmentId);

    try {
      const res = await fetch(`${BACKEND_URL}/submission/assignment-submission`, {
        method: "POST",
        headers: {
          Authorization:
            "Bearer " +
  ...
  async function handleUndoSubmission() {
    ...
    const warn = isClosed
      ? "If you unsubmit now, you will NOT be able to submit again because submissions are closed.\n\nContinue?"
      : "Are you sure you want to undo your submission?";

    if (!confirm(warn)) return;

    try {
      const res = await fetch(`${BACKEND_URL}/submission/${submission._id}/unsubmit`, {
        method: "DELETE",
        headers: {
          Authorization:
            "Bearer " +
    ...
    <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-white/20">
      ...
      <div className="p-8">
        ...
        {submission && submission.status === "submitted" ? (
          <div className="space-y-6">
            ...
            {isClosed && (
              <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-xl text-yellow-800">
                ⚠️ Submissions are currently closed. If you unsubmit now, you won’t be able to submit again.
              </div>
            )}
          ...
          // NOT submitted yet: form OR closed banner
          <>
            {isClosed ? (
              <div className="p-6 bg-gray-50 rounded-2xl border border-gray-200 text-center">
                ...
                <h3 className="font-semibold text-gray-800">Submissions are closed</h3>
                <p className="text-sm text-gray-600 mt-1">
                  You cannot submit this assignment now.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                <div className="space-y-4">
                  <label className="block text-lg font-bold text-gray-800 items-center gap-2">
                    <Upload className="w-5 h-5 text-indigo-600 inline-block mr-2" />
                    Upload Files
                  </label>
                  <div
                    className={`relative border-2 border-dashed rounded-2xl p-8 text-center transition-all duration-300 ${
                      dragActive ? 'border-indigo-400 bg-indigo-50 scale-105'
                ...
                <div className="space-y-4">
                  <label className="block text-lg font-bold text-gray-800 items-center gap-2">
                    <LinkIcon className="w-5 h-5 text-emerald-600 inline-block mr-2" />
                    Add Links (Optional)
                  </label>
        ...
        {/* Plagiarism Result (inline) */}
        {plagiarismResult && (
          <div className="mb-6 mt-6 p-6 bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-200 rounded-2xl shadow space-y-2">
            <div className="flex items-center gap-3 mb-2">
