        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-gray-600">{totalMaterials} materials available</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          {visibleGroups.map((group) => (
            <div key={group.topic._id ?? "uncategorized"} className="group">
              ...
              {/* Materials List (simple view) */}
              <div className={`grid gap-3 ${viewMode === "card" ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"}`}>
                {group.materials.map((material) => (
                  <div
                    key={material._id}
                    className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col sm:flex-row items-center sm:items-stretch px-5 py-4 justify-between group/card"
                  >
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <FileText className="w-7 h-7 text-blue-600" />
                      <div className="flex-1 min-w-0">
                        <div className="font-semibold text-base text-gray-900 truncate">{material.title}</div>
                    ...
                    <div className="mt-3 sm:mt-0 sm:ml-6">
                      <button
                        onClick={() => {
                          console.log("Clicked material id:",material._id); // <--- LOG HERE
