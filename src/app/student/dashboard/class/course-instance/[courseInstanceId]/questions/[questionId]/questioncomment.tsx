...
export default function QuestionCommentSection({ questionId }: Props) {
  const { user } = useUser();
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`${BACKEND_URL}/comment/question-comments/${questionId}`)
      .then((r) => r.json())
      .then(setComments)
      .catch(() => setComments([]))
  ...
  async function handlePostComment() {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/comment/question-comments/${questionId}`,
        {
          method: "POST",
          headers: {
  ...
  async function handleDelete(commentId: string) {
    try {
      const res = await fetch(
        `${BACKEND_URL}/comment/question-comments/${commentId}`,
        {
          method: "DELETE",
          headers: {
            Authorization:
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          <TiptapEditor
            content={comment}
            onChange={setComment}
            placeholder="Share your thoughts or ask questions about this question..."
          />
