...
function RecipientsPanel({
  students, visibleTo, setVisibleTo, isAllSelected, onClose
}: {
  students: any[];
  visibleTo: string[];
  ...
  return (
    <div className="bg-white rounded-lg p-6 max-w-md w-full shadow-lg border">
      ...
      <div className="flex justify-end pt-4">
        <button className="px-4 py-2 bg-blue-600 text-white rounded" onClick={onClose}>
          Done
        </button>
      </div>
    </div>
...
useEffect(() => {
  ...
  (async () => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/assignment/${assignmentId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await res.json();
    if (!data.assignment) {
      toast.error(data.error || "Assignment not found.");
      setLoading(false);
      return;
    }
    setTitle(data.assignment.title || "");
    setPoints(data.assignment.points || "");
    setDueDate(
      data.assignment.dueDate
        ? new Date(data.assignment.dueDate).toISOString().slice(0, 16)
        : ""
    );
setMedia(data.assignment.media as FileEntry[] || []);
setDocuments(data.assignment.documents as FileEntry[] || []);
    setLinks(data.assignment.links || []);
    setVideos(data.assignment.youtubeLinks || []);
    setTopic(data.assignment.topic || "");
    setVisibleTo(data.assignment.visibleTo || []);
    setMutedStudents(data.assignment.mutedStudents || []);
    if (editor) editor.commands.setContent(data.assignment.content || "");
    setLoading(false);
  })();
...
function removeExistingDoc(url: string) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!points) { toast.error("Points are required!"); return; }
    ...
    try {
      ...
      formData.append("visibleTo", isAllSelected ? JSON.stringify([]) : JSON.stringify(visibleTo));
      // Always send topic (may be empty string for clearing topic)
  console.log("Current topic value being sent:", topic);
formData.append("topic", topic);
console.log("FormData topic (should match above):", formData.get("topic"));
      ...
      if (mediaToRemove.length) formData.append("mediaToRemove", JSON.stringify(mediaToRemove));
      if (docsToRemove.length) formData.append("documentsToRemove", JSON.stringify(docsToRemove));

      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/assignment/${assignmentId}`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
            ...
            <div className="flex-1 min-w-0 flex flex-col max-h-[68vh] overflow-y-auto">
              ...
              <div className="border-2 rounded-lg bg-gray-50 focus-within:border-blue-300 mb-3">
                ...
                <div className="flex gap-2 p-3 pt-2 border-t bg-white rounded-b-lg">
                  {/* ... (editor toolbar - same as your current code) ... */}
                  <button type="button" onClick={() => editor?.chain().focus().toggleBold().run()} className={`px-3 py-1 rounded text-sm font-bold transition ${editor?.isActive("bold") ? "bg-blue-600 text-white border border-blue-700 shadow font-extrabold scale-95" : "hover:bg-blue-100 hover:text-blue-700"}`}>B</button>
                  <button type="button" onClick={() => editor?.chain().focus().toggleItalic().run()} className={`px-3 py-1 rounded text-sm italic transition ${editor?.isActive("italic") ? "bg-purple-600 text-white border border-purple-700 shadow scale-95" : "hover:bg-purple-100 hover:text-purple-700"}`}>I</button>
                  <button type="button" onClick={() => editor?.chain().focus().toggleUnderline().run()} className={`px-3 py-1 rounded text-sm underline transition ${editor?.isActive("underline") ? "bg-green-600 text-white border border-green-700 shadow scale-95" : "hover:bg-green-100 hover:text-green-700"}`}>U</button>
                  <button type="button" onClick={() => editor?.chain().focus().toggleBulletList().run()} className={`px-3 py-1 rounded text-sm hover:bg-gray-100 ${editor?.isActive("bulletList") ? "bg-blue-100 text-blue-700" : ""}`}>• List</button>
                  <button type="button" onClick={() => editor?.chain().focus().toggleOrderedList().run()} className={`px-3 py-1 rounded text-sm hover:bg-gray-100 ${editor?.isActive("orderedList") ? "bg-blue-100 text-blue-700" : ""}`}>1. List</button>
                  <button type="button" onClick={() => editor?.chain().focus().clearNodes().unsetAllMarks().run()} className="px-3 py-1 rounded text-sm hover:bg-gray-100 text-red-600">Clear</button>
                </div>
              </div>
              ...
              <div className="flex gap-4 mb-2">
                ...
                <label className="cursor-pointer group">
                  <input type="file" hidden multiple accept="image/*" onChange={(e) => { const files = Array.from(e.target.files || []); setImages((prev) => [...prev, ...files]); }} />
                  <div className="w-12 h-12 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center group-hover:border-blue-500 group-hover:bg-blue-50"><ImageIcon size={20} className="text-gray-500 group-hover:text-blue-500" /></div>
                  <div className="text-xs text-center mt-1 text-gray-500">Images</div>
                </label>
                {/* YouTube */}
                <button type="button" onClick={() => { setYoutubeInput(""); setYoutubeError(""); setModalType("youtube"); }} className="group">
                  <div className="w-12 h-12 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center group-hover:border-red-500 group-hover:bg-red-50"><YoutubeIcon size={20} className="text-gray-500 group-hover:text-red-500" /></div>
                  <div className="text-xs text-center mt-1 text-gray-500">YouTube</div>
                </button>
                {/* Add new documents */}
                <label className="cursor-pointer group">
                  <input type="file" hidden multiple accept=".pdf,.doc,.docx,.ppt,.pptx,.csv,.xls,.xlsx,.txt" onChange={(e) => { const files = Array.from(e.target.files || []); setDocs((prev) => [...prev, ...files]); }} />
                  <div className="w-12 h-12 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center group-hover:border-green-500 group-hover:bg-green-50"><FileTextIcon size={20} className="text-gray-500 group-hover:text-green-500" /></div>
                  <div className="text-xs text-center mt-1 text-gray-500">Documents</div>
                </label>
                {/* Links */}
                <button type="button" onClick={() => { setLinkInput(""); setLinkError(""); setModalType("link"); }} className="group">
                  <div className="w-12 h-12 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center group-hover:border-purple-500 group-hover:bg-purple-50"><LinkIcon size={20} className="text-gray-500 group-hover:text-purple-500" /></div>
      ...
      <Trash2 size={14} />
            ...
            <div className="w-[340px] flex-shrink-0 h-fit flex flex-col gap-5 bg-gray-50 rounded-2xl border border-gray-200 p-6">
              ...
              <div>
                ...
                <button type="button" className="w-full flex items-center justify-between bg-white border rounded-full px-4 py-2 hover:border-blue-400 transition" onClick={() => setShowRecipients(true)}>
              ...
              <div>
                <div className="text-gray-600 font-semibold mb-1 flex items-center gap-1">
                  Topic
                  <button type="button" onClick={() => setShowTopicInput(v => !v)} className="ml-1 text-blue-500 hover:underline">
                    <Plus size={16} />
                  </button>
                </div>
                {showTopicInput ? (
                  <div className="flex gap-2 mt-1">
                    <input className="flex-1 bg-gray-100 p-2 rounded border" placeholder="New topic name" value={newTopic} onChange={e => setNewTopic(e.target.value)} />
                    <button type="button" className="px-2 rounded bg-blue-600 text-white" onClick={handleAddTopic}>Add</button>
                    <button type="button" className="p-2 hover:bg-gray-100 rounded" onClick={() => { setShowTopicInput(false); setNewTopic(""); }} title="Cancel">
                      <X size={18} />
                    </button>
                  </div>
                ) : (
                  <select className="w-full bg-white rounded p-3 mt-1 border" 
              ...
              <div>
                ...
                <input type="number" min={1} max={1000} required className="w-full bg-white rounded p-3 border" placeholder="Total Points (e.g., 10)" value={points} onChange={e => setPoints(Number(e.target.value) || "")} />
              </div>
              {/* Due Date */}
              <div>
                <div className="text-gray-600 font-semibold mb-1">Due Date</div>
                <input type="datetime-local" required className="w-full bg-white rounded p-3 border" value={dueDate} onChange={e => setDueDate(e.target.value)} />
              </div>
              ...
              <div className="mt-6">
                ...
                <button type="button" className="w-full border rounded-full px-4 py-2 bg-white text-gray-700 hover:border-blue-400 transition mb-1" onClick={() => setShowMuteModal(true)}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {images.map((file, i) => (
                      <div key={i} className="flex items-center gap-2 bg-white p-2 rounded shadow border">
                        <img src={URL.createObjectURL(file)} alt={file.name} className="w-20 h-16 object-cover rounded cursor-pointer"
                          onClick={() => {
                            setPreviewContent(
                              <div className="p-4">
                                <img src={URL.createObjectURL(file)} alt={file.name} className="max-w-full max-h-[80vh] object-contain mx-auto" />
                                <p className="text-center mt-2 text-gray-600">{file.name}</p>
                              </div>
                            );
                            setModalType("preview");
                          }} />
                        <button type="button" onClick={() => setImages(imgs => imgs.filter((_, idx) => idx !== i))} className="text-red-500"><Trash2 size={16} /></button>
                      </div>
                    ))}
                  </div>
                </div>
                      ...
                      return (
                        <div key={i} className="flex items-center gap-2 bg-white p-2 rounded shadow border">
                          ...
                          <button type="button" onClick={() => {
                            ...
                            setModalType("preview");
                          }} className="text-blue-600 hover:underline"><ZoomIn size={16} /></button>
                          <button type="button" onClick={() => setDocs(d => d.filter((_, idx) => idx !== i))} className="text-red-500"><Trash2 size={16} /></button>
                        </div>
                      );
                    })}
                  </div>
                        ...
                        <div key={i} className="bg-white rounded shadow border overflow-hidden flex">
                          <div className="cursor-pointer"
                            onClick={() => {
                              setPreviewContent(
                                <div className="p-4">
                                  <iframe width="850" height="480" src={embedUrl} title="YouTube video" allowFullScreen className="rounded" />
                                </div>
                              );
                              setModalType("preview");
                            }}>
                            <iframe width="180" height="100" src={embedUrl} title="YouTube video" allowFullScreen className="pointer-events-none" />
                          </div>
                          <button type="button" onClick={() => setVideos(vid => vid.filter((_, idx) => idx !== i))} className="text-red-500 p-2"><Trash2 size={16} /></button>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {links.map((url, i) => (
                      <div key={i} className="bg-white p-2 rounded shadow border flex items-center gap-2">
                        ...
                        <button type="button" onClick={() => setLinks(l => l.filter((_, idx) => idx !== i))} className="text-red-500"><Trash2 size={16} /></button>
                      </div>
                    ))}
                  </div>
                </div>
