...
import {
  ...
  Play, ExternalLink, Trash2, Plus, Users2
} from "lucide-react";
...
export default function CourseMaterialForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Correct: send as 'media' and 'documents'
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/course-materials/`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
      ...
      setTitle(""); setDocs([]); setImages([]); setLinks([]); setVideos([]); setVisibleTo([]);
            ...
            <div className="flex-1 min-w-0 flex flex-col max-h-[68vh] overflow-y-auto">
              ...
              <div className="border-2 rounded-lg bg-gray-50 focus-within:border-blue-300 mb-3">
                <div className="p-3 pb-0">
                  <div className="text-sm text-gray-600 mb-2">Write your material</div>
                ...
                <div className="flex gap-2 p-3 pt-2 border-t bg-white rounded-b-lg">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setEditorState(x => x + 1); // <-- force re-render right after toggling
                    }}
...
<div className="mt-6">
  <div className="flex items-center gap-2 mb-2">
    <label className="font-semibold text-gray-600">Customer</label>
    <button
      type="button"
      ...
      : "No students muted"}
        ...
        <div className="border-t bg-white py-4 px-8 flex justify-between items-center sticky bottom-0 z-20">
          <button
            ...
            onClick={() => {
              setTitle(""); setImages([]); setDocs([]); setVideos([]); setLinks([]); setVisibleTo([]);
              setTopic(""); editor?.commands.clearContent(); setMessage(null);
              onSuccess?.();
            }}
          >
            Cancel
