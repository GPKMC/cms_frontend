...
// --- Main Edit Form ---
export default function CourseMaterialEditForm({
  materialId,
  courseInstanceId,
  onSuccess,
}: {
...
const [documents, setDocuments] = useState<FileEntry[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/course-materials/material/${materialId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      setTitle(data.material.title || "");
      setContent(data.material.content || "");
     setMedia(data.material.media as FileEntry[] || []);
setDocuments(data.material.documents as FileEntry[] || []);

      setLinks(data.material.links || []);
      setVideos(data.material.youtubeLinks || []);
      setTopic(data.material.topic || "");
      setVisibleTo(data.material.visibleTo || []);
      setMutedStudents(data.material.mutedStudents || []);
      editor?.commands.setContent(data.material.content || "");
      setLoading(false);
    })();
 ...
 function removeExistingMedia(url: string) {
  setMediaToRemove(to => [...to, url]);
  setMedia(curr => curr.filter(item => item.url !== url));
}
function removeExistingDoc(url: string) {
  setDocsToRemove(to => [...to, url]);
  setDocuments(curr => curr.filter(item => item.url !== url));
}
...
formData.append("topic", topic); // topic will be "" for "No topic"
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/course-materials/${materialId}`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
            ...
            <div className="flex-1 min-w-0 flex flex-col max-h-[68vh] overflow-y-auto">
              ...
              <div className="border-2 rounded-lg bg-gray-50 focus-within:border-blue-300 mb-3">
                <div className="p-3 pb-0">
                  <div className="text-sm text-gray-600 mb-2">Edit your material</div>
  ...
  >Clear</button>
              ...
              <div className="flex gap-4 mb-2">
                ...
                <button
                  type="button"
                  onClick={() => { setYoutubeInput(""); setYoutubeError(""); setModalType("youtube"); }}
                  className="group"
                >
                ...
                <button
                  type="button"
                  onClick={() => { setLinkInput(""); setLinkError(""); setModalType("link"); }}
                  className="group"
                >
  ...
  <div key={i} className="relative flex items-center gap-2 bg-white p-2 rounded shadow border">
    ...
    <button type="button" onClick={() => removeExistingDoc(item.url)} className="text-red-500">
      <Trash2 size={14} />
          ...
          {/* Preview Bar (newly added) */}
          <div className="w-full px-8 pb-5 flex flex-col gap-6 items-start  bg-gray-50 min-h-[80px] max-h-[180px] overflow-x-auto overflow-y-auto">
            {/* New Images Row */}
            {images.length > 0 && (
