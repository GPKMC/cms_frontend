...
import { authHeaders, buildAdminListUrl, getApiBase, getAuthToken, getBackendBase } from "./utlis";
import type { RoleFilter, StatusFilter } from "./utlis";
          ...
          <div className="inline-flex overflow-hidden rounded-lg ring-1 ring-zinc-200">
            {([
              ...
              <button
                ...
                className={classNames(
                  "px-3 py-1.5 text-sm",
                  roleFilter === (b.key as RoleFilter) ? "bg-zinc-900 text-white" : "bg-white text-zinc-700 hover:bg-zinc-50",
                  i !== 0 && "border-l border-zinc-200"
                )}
                onClick={() => setRoleFilter(b.key as RoleFilter)}
              >
          ...
          <div className="inline-flex overflow-hidden rounded-lg ring-1 ring-zinc-200">
            {statusTabs.map((b, i) => (
              <button
                ...
                className={classNames(
                  "px-3 py-1.5 text-sm",
                  statusFilter === (b.key as StatusFilter) ? "bg-zinc-900 text-white" : "bg-white text-zinc-700 hover:bg-zinc-50",
                  i !== 0 && "border-l border-zinc-200"
                )}
...
function runSelfTests(api: string) {
  try {
    const isProd =
    ...
    let envOk = true;
    try {
      void (typeof process !== "undefined" && (process as any).env?.NEXT_PUBLIC_BACKEND_URL);
    } catch {
      envOk = false;
    }
    ...
    const roles: RoleFilter[] = ["all","teacher","student"];
    const statuses: StatusFilter[] = ["pending","approved","rejected","cancelled","all"];
    const built: string[] = [];
    ...
    if (typeof console !== "undefined") console.table(tests);
  } catch {
    // swallow
  }
}
