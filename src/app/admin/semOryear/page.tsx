...
type Semester = {
  _id: string;
  name: string; // << Use 'name' instead of 'semesterName'
  faculty: FacultyObj | string;
  batch?: string;
  startDate?: string;
  endDate?: string;
  courses: CourseObj[];
...
export default function SemesterOrYearList() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        const faculties = Array.isArray(res.data) ? res.data : res.data.faculties || [];
  ...
  useEffect(() => {
    ...
    axios
      .get(batchUrl, { params: { faculty: facultyId }, ...axiosConfig })
      .then((res) => {
  ...
  }, [search, facultyId, batchId, limit, page, token]);
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    if (!token) {
      setDeleteError("Unauthorized: Please login.");
    ...
    setDeleteError(null);
    try {
      const res = await axios.delete(`${url}/${deleteId}`, axiosConfig);
      if (res.data.success) {
            ...
            {semesters.length === 0 ? (
              ...
              semesters.map((sem, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap border-r border-gray-300">{sem.name}</td>
                  <td className="px-4 py-3 whitespace-nowrap border-r border-gray-300">
                    {typeof sem.faculty === "object"
                      ? `${sem.faculty.code} (${sem.faculty.name})`
                      : sem.faculty}
                  </td>
             
                  {/* <td className="px-4 py-3 whitespace-nowrap border-r border-gray-300">
                    {sem.courses && sem.courses.length
                      ? sem.courses
                          .map((c) =>
                            typeof c === "object"
                              ? `${c.code} (${c.name})`
