...
export default function AnnouncementAdminMailbox() {
  ...
  async function fetchFolderCounts() {
    try {
      const res = await fetch(EP.folderCounts, { headers: { ...authHeaders(false) }, cache: "no-store" });
      ...
      // fallback: compute from currently loaded rows
      const counts = allRows.reduce<FolderCounts>(
        (acc, a) => {
          const st = status(a);
          acc.all++;
  ...
  async function archive(id: string) {
    try {
      await fetch(EP.archive(id), { method: "POST", headers: authHeaders(true), body: JSON.stringify({}) });
  ...
  async function unarchive(id: string) {
    try {
      await fetch(EP.unarchive(id), { method: "POST", headers: authHeaders(true), body: JSON.stringify({}) });
      await fetchList(); await fetchFolderCounts();
