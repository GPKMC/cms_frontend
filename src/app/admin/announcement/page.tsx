...
function validateAdd(kind: 'image' | 'doc', currentCount: number, selected: File[]) {
  const maxCount = kind === 'image' ? MAX_IMG : MAX_DOC;
  if (currentCount + selected.length > maxCount) {
    toast.error(`Too many ${kind === 'image' ? 'images' : 'files'} (max ${MAX_DOC}).`);
    return false;
  }
...
export default function AnnouncementCreatePage() {
  ...
  async function fetchBatchesForFaculty(fid: string) {
    ...
    if (!fac?.code) {
      setBatches([]);
      setSelectedBatchIds([]);
      toast.error('Selected faculty has no code; cannot load batches.');
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      // API shape: { success, batches, totalCount } — fallback to common shapes
      const raw: any[] = Array.isArray(data?.batches)
        ? data.batches
        : Array.isArray(data)
        ? data
        : data.data || [];
      // normalize names once so UI is clean
      ...
      setBatches([]);
      toast.error('Failed to load batches for the selected faculty');
    } finally {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>, forcePublished?: boolean) {
    ...
    if (audienceMode === 'faculty') {
      if (selectedFacultyIds.length === 0) {
        toast.error('Select at least one faculty (or switch to All).');
        return;
      }
    }
    ...
    if (audienceMode === 'batches') {
      if (!batchFacultyId) {
        toast.error('Choose a faculty to load its batches.');
        return;
      }
      if (selectedBatchIds.length === 0) {
        toast.error('Select at least one batch (or use Faculty mode for all batches under that faculty).');
        return;
      }
    }
        ...
        <form onSubmit={(e) => handleSubmit(e)} className="space-y-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              {/* Faculty mode: multi-select FACULTIES (targets all batches under those faculties) */}
              {audienceMode === 'faculty' && (
              ...
              {/* Batches mode: first pick a faculty (single), then we load only its batches */}
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-green-400 transition-colors duration-200">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!files.length) return;
                      if (!validateAdd('image', imageFiles.length, files)) {
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-blue-400 transition-colors duration-200">
                  <input
                    ...
                    onChange={(e) => {
                      const files = Array.from(e.target.files || []);
                      if (!files.length) return;
                      if (!validateAdd('doc', docFiles.length, files)) {
                        e.currentTarget.value = '';
                        return;
                      }
