...
// safer for partial input while typing
const toMinutesSafe = (s?: string) => {
...
const toAmPm = (s?: string) => {
  ...
  const mm = (Number(m) || 0).toString().padStart(2, "0");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <button
          ...
          className="mb-4 inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm text-gray-700 shadow-sm transition-all hover:bg-gray-50 hover:shadow"
        >
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl border border-white/50 overflow-hidden">
          ...
          <div className="bg-gray-50/50 px-6 py-4 flex items-center justify-between border-t border-gray-100">
            <div className="flex-1">
              {message && (
                <div
                  className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium animate-pulse ${
      ...
      <div className="p-4">
        {slots.length === 0 ? (
          ...
          <div className="space-y-3">
            {slots.map((s, index) => {
              const startMins = toMinutesSafe(s.start);
              const endMins = toMinutesSafe(s.end);
              const valid = Number.isFinite(startMins) && Number.isFinite(endMins) && (endMins as number) > (startMins as number);

              let durationLabel = "—";
              if (valid) {
                const duration = (endMins as number) - (startMins as number);
                const hours = Math.floor(duration / 60);
                const mins = duration % 60;
                durationLabel = hours > 0 ? `${hours}h ${mins ? `${mins}m` : ""}`.trim() : `${mins}m`;
              }
                  ...
                  <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-all duration-200 hover:border-indigo-300">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-4">
                      ...
                      <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            />
                            <span className="whitespace-nowrap rounded-md bg-indigo-50 px-2 py-1 text-xs font-medium text-indigo-700">
                              {toAmPm(s.start) || "—"}
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="text-center min-w-[90px]">
                        ...
                        <p className={`text-sm font-bold ${valid ? "text-indigo-600" : "text-gray-400"}`}>
                          {durationLabel}
                        </p>
                      </div>
                      ...
                      <button
                        ...
                        className="w-10 h-10 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg flex items-center justify-center transition-all duration-200 hover:scale-105"
                        title="Remove slot"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
